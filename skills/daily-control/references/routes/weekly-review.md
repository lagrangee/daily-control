...
Use the configured week start and review the most recently closed interval by default. State the date range. Ask only when the intended interval is ambiguous or the user requests another range.

Complete when one closed interval is explicit.
...
Rebuild `context/now.md` from the committed Weekly and owner records, including `Updated` and `Derived from` links. Read it back.

Complete when the summary reflects the new working set without becoming the only record of a decision.
