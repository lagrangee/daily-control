...
5. **Adaptive Open** — use a stable day with confirmed direction and adequate saved Evidence. Pass when Open can stay brief and leave both Daily and summary unchanged; bare help recognizes judgment-only content without running a route. Then request a plan conflicting with one configured Control Policy field: the Agent surfaces the conflict and records an override only after confirmation. For re-entry, use older-format records with a stalled project, an interruption, and partial reading Evidence. Pass when the Agent reads relevant history, separates facts from uncertainty, accepts a judgment or recovery direction without forcing a plan, preserves old content, and creates no backfill or automatic Refresh. Explicit decisions need no repeated field confirmation; new consequential interpretations do.
...
8. **Shutdown and intervention feedback** — use an older-format Daily with a recovery or judgment-only direction and insufficient saved Evidence. Pass when Shutdown presents a bounded draft before a general self-report, asks only for material missing interpretation, and invents neither completion nor cross-surface conversation history. Refresh remains user-requested. After explicit user correction, it writes only Shutdown and reads back durable history before any needed summary update. In a later Open, provide feedback that a recorded intervention did not help: pass when the Agent recovers the prior decision and discusses revising its premise rather than simply repeating the advice. Missing feedback alone remains uncertainty.
9. **Weekly review** — invoke weekly-review for a closed interval containing that feedback. Pass when it uses existing review space for supported lessons about the loop without a fixed questionnaire, links relevant Areas, Projects, and Routines, and applies a proposed Control Policy, priority, or Controller Style change only after separate confirmation. Unconfirmed owner records remain unchanged.
