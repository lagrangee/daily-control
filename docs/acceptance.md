...
2. **Create** — invoke `/daily-control setup` in the empty fixture. Pass when the Agent previews the target, creates the scaffold only after confirmation, includes an unconfigured Control Policy and the Daily Policy overrides surface, and reads back required files.
